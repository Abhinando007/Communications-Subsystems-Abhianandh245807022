...
OUTPUT_BITS_NPY = os.path.join(OUTPUT_DIR, "doppler5000Hz10db.npy")
OUTPUT_BITS_TXT = os.path.join(OUTPUT_DIR, "doppler5000Hz10db.txt")
OUTPUT_LLRS     = os.path.join(OUTPUT_DIR, "recovered_llrs510.npy")
...
def recover_bits(rx_path, meta_path, verbose=True, plot_doppler=True):
    ...
    if verbose:
        print(f"Saved bits -> {OUTPUT_BITS_NPY}")
        print(f"Saved bits (txt) -> {OUTPUT_BITS_TXT}")
...
if __name__ == "__main__":
    if in_notebook():
        ...
        parser.add_argument("--out_dir", default=OUTPUT_DIR, help="Output folder")
        args = parser.parse_args()

        # Update output paths
        os.makedirs(args.out_dir, exist_ok=True)
        OUTPUT_BITS_NPY = os.path.join(args.out_dir, "doppler5000Hz10db.npy")
        OUTPUT_BITS_TXT = os.path.join(args.out_dir, "doppler5000Hz10db.txt")
        OUTPUT_LLRS     = os.path.join(args.out_dir, "recovered_llrs5k10.npy")
